...
const FeatureCard = ({ icon, title, content, index }) => (
  <div
    className={`flex flex-row p-6 text-black rounded-[20px] ${
      index !== features.length - 1 ? "mb-6" : "mb-0"
    } feature-card`}
  >
    <div
      className={`w-[70px] h-[70px] rounded-full bg-[#41CB5B] ${styles.flexCenter} bg-dimBlue`}
    >
      <img src={icon} alt={title} className="w-[100%] h-[50%] object-contain" />
    </div>
...
const styles = {
  ...
  heading2:
    "font-semibold xs:text-[48px] text-[36px] text-[#41CB5B] xs:leading-[76.8px] leading-[66.8px] w-full",
...
const CardComponent = () => {
  return (
    <section id="features" className={`${layout.section} px-6 md:px-24`}>
      <div className={layout.sectionInfo}>
        <h2 className={styles.heading2}>
          Experience Reliable Car, <br className="sm:block hidden" /> Care with
          GasUpp.
        </h2>
        <p className={`${styles.paragraph} max-w-[470px] mt-5`}>
          GasUpp is dedicated to providing fast and reliable gas delivery and
          automotive services right to your doorstep. Our mission is to make car
          care as effortless as possible, offering convenience and peace of mind
          with every service.
        </p>
